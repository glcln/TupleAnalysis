...
float K_data2017(2.30), C_data2017(3.17);
    ...
    TH2D* tmp = new TH2D(strmass.c_str(),";p [GeV];I_{h} [MeV/cm]",200,0,2000,100,0,10);
...
void crossHistosEtaBinning(TH2D* res, TH2D* eta_1oP, TH2D* ih_eta)
{
    ...
    {
        TH1D* p = (TH1D*) eta_1oP->ProjectionX("proj_p",i,i+1);
        TH1D* ih = (TH1D*) ih_eta->ProjectionY("proj_ih",i,i+1);
        scale(p);
