...
# Collect line numbers of all commented lines containing "MuonEG2024"
# Format of grep -n output: <line_number>:<line_content>
mapfile -t MuonEG_LINES < <(grep -n '^#.*MuonEG2024' "$CONFIG" | cut -d: -f1)

N_LINES=${#MuonEG_LINES[@]}
echo "Found $N_LINES MuonEG2024 samples"

# Loop over each MuonEG2024 line
for ((i=0; i<N_LINES; i++)); do
    LINE=${MuonEG_LINES[$i]}

    echo "========================================"
    echo "Running MuonEG2024 sample $((i+1)) / $N_LINES"
    ...
    # 2) Uncomment the selected MuonEG2024 line
    sed -i "${LINE}s/^#//" "$CONFIG"
